...
<form action="/hal_cinema_2.0/public/admin/{{ $show_id }}/update/confirm" method="POST">
	...
	<div class="form-group">
		...
		<input type="text" name="movie_title" id="movie_title" class="form-control" value="{{ $movie_title }}" required>
	</div>
	<div class="form-group">
		<label for="start_datetime">start_datetime</label>
		<br>
		<input type="datetime-local" name="start_datetime" value="{{ $start_datetime }}" min="{{ date('Y-m-d') }}T{{ date('H:i') }}" required>
		<input type="hidden" name="start_datetime_old" value="{{ $start_datetime }}">
	</div>
	<div class="form-group">
		<label for="cleaning_time">上映前後1セット分のcleaning_time(minutes)</label>
		<input type="number" name="cleaning_time" id="cleaning_time" class="form-control" value="{{ $cleaning_time }}" required>
	</div>
	<div class="form-group">
